<?php
ob_start();
session_start();
date_default_timezone_set('Europe/Istanbul');

require_once 'ayar/baglan.php';
require_once 'ayar/fonksiyon.php';

if (isset($_POST['sifremiunuttum'])) {
	
	$kullanicisor=$db->prepare("SELECT * FROM kullanici where kullanici_mail=:mail and kullanici_username=:kullanici_username and kullanici_guvenlikkod=:kullanici_guvenlikkod ");
	$kullanicisor->execute(array(
		'mail' =>htmlspecialchars($_POST['kullanici_mail']),
		'kullanici_username' =>htmlspecialchars($_POST['kullanici_username']) ,
		'kullanici_guvenlikkod' =>htmlspecialchars($_POST['kullanici_guvenlikkod'])
	));
	$say=$kullanicisor->rowCount();

	$kullanici_mail=htmlspecialchars($_POST['kullanici_mail']);
   $kullanici_ad = htmlspecialchars($_POST['kullanici_username']);

	if ($say==0) {
		
		Header("Location:sifremi-unuttum.php?giris=hatalibilgi");
		exit;

	} else {

		$kullanicicek=$kullanicisor->fetch(PDO::FETCH_ASSOC);

		$uretilensifre=uniqid();
		$sifrekaydet=md5($uretilensifre);

		//Veritabanı kaydını yap

		$sifreguncelle=$db->prepare("UPDATE kullanici SET


			kullanici_password=:kullanici_password

			WHERE kullanici_mail='$kullanici_mail'");


		$update=$sifreguncelle->execute(array(


			'kullanici_password' => $sifrekaydet

		));


		//Varitabanı kaydı bitir
		

	//Mail Gönderim Başlat

		$ayarsor=$db->prepare("SELECT * FROM ayar where ayar_id=0");
	$ayarsor->execute();	
		
		$ayarcek=$ayarsor->fetch(PDO::FETCH_ASSOC);
		
		

		$yenisifre="Yeni Şifreniz : ".$uretilensifre;



require("mailphp/class.phpmailer.php"); // PHPMailer dosyamizi çagiriyoruz
$mail = new PHPMailer(); // Sinifimizi $mail degiskenine atadik
$mail->IsSMTP(true);  // Mailimizin SMTP ile gönderilecegini belirtiyoruz


$mail->From     = $ayarcek['ayar_smtpuser'];//"admin@localhost"; //Gönderen kisminda yer alacak e-mail adresi
$mail->Sender   = $ayarcek['ayar_smtpuser'];//"admin@localhost";//Gönderen Mail adresi
//$mail->ReplyTo  = ($_POST["mailiniz"]);//"admin@localhost";//Tekrar gönderimdeki mail adersi

$mail->AddReplyTo=($ayarcek['ayar_smtpuser']);//"admin@localhost";//Tekrar gönderimdeki mail adersi
$mail->AddAddress($kullanici_mail); // Mail gönderilecek adresleri ekliyoruz.
//$mail->AddAttachment('naci.jpg');
$mail->FromName = $ayarcek['ayar_sunucuad'];//"PHP Mailer";//gönderenin ismi
$mail->Host     = $ayarcek['ayar_smtphost'];//"localhost"; //SMTP server adresi
$mail->SMTPAuth = true; //SMTP server'a kullanici adi ile baglanilcagini belirtiyoruz
$mail->Port     = $ayarcek['ayar_smtpport']; //Natro SMPT Mail Portu
$mail->CharSet = 'UTF-8'; //Türkçe yazı karakterleri için CharSet  ayarını bu şekilde yapıyoruz.
$mail->Username = $ayarcek['ayar_smtpuser'];//"admin@localhost"; //SMTP kullanici adi
$mail->Password = $ayarcek['ayar_smtpsifre'];//""; //SMTP mailinizin sifresi
//$mail->WordWrap = 50;
//$mail->IsHTML(true); //Mailimizin HTML formatinda hazirlanacagini bildiriyoruz.
$mail->Subject  = "Sifre Sifirlama";//"Deneme Maili"; // Mailin Konusu Konu
//Mailimizin gövdesi: (HTML ile)
$mail->Body = $yenisifre;
//$mail->AltBody = $text_body;


if ($mail->Send()) {
	
	Header("Location:giris.php?mail=ok");
	exit;

}

else {

	Header("Location:sifremi-unuttum.php?durum=mailno");
	exit;

	/*echo "Mail Gönderimi Başarısız"; echo "<br>";
	echo "Hata: ".$mail->ErrorInfo;
	*/


}



/*$mail->ClearAddresses();
$mail->ClearAttachments();
$mail->AddAttachment('images.png'); //mail içinde resim göndermek için
$mail->addCC('mailadi@alanadiniz.site');// cc email adresi
$mail->addBCC('mailadi@alanadiniz.site');// bcc email adresi
$mail->AddAddress("mailadi@alanadiniz.site"); // Mail gönderilecek adresleri ekliyoruz.
$mail->AddAddress("mailadi@alanadiniz.site"); // Mail gönderilecek adresleri ekliyoruz. Birden fazla ekleme yapılabilir.
$mail->Send();
$mail->ClearAddresses();
$mail->ClearAttachments();
*/

//Mail Gönderim Bitir





}



}




?>